...
open class HNComment: BaseComment, Identifiable {
    ...
    override public init(level: Int, replyTo: BaseComment?) {
    ...
    public convenience init?(fromHtml html: String, withParsingConfig parseConfig: [String: Any], levelOffset: Int = 0) {
        ...
        if html.contains((commentDict!["Upvote"] as! [String: String])["R"]!) {
            ...
            if upvoteString != nil {
                upvoteUrl = upvoteString!.replacingOccurrences(of: "&amp;", with: "&")
            }
            ...
            if html.contains((commentDict!["Downvote"] as! [String: String])["R"]!) {
                ...
                if downvoteString != nil {
                    downvoteUrl = downvoteString!.replacingOccurrences(of: "&amp;", with: "&")
                }
            }
        }
        ...
        id = cDict["CommentId"] as? String ?? ""
        text = cDict["Text"] as? String ?? ""
        username = cDict["Username"] as? String ?? ""
        isOPNoob = HNUser.cleanNoobUsername(username: &(username!))
        created = cDict["Time"] as? String ?? ""
        replyUrl = cDict["ReplyUrl"] as? String ?? ""

        if id != "", html.contains("<a id=\'un_\(id!)") { // TODO: put that in the configFie
            upvoted = true
        }
    }
