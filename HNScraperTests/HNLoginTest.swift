...
class HNLoginTest: XCTestCase {
    ...
    func testGoodLogin() {
        ...
        HNLogin.shared.login(username: "abdurhtl", psw: "!Bullshit?Psw$", completion: { (user, cookie, _) -> Void in
            XCTAssertNotNil(cookie)
            XCTAssertEqual(user?.username, "abdurhtl")
            exp.fulfill()
        })
        let exp2 = expectation(description: "Correct login with testHNScrapper")
        HNLogin.shared.login(username: "testHNScrapper", psw: "&$!?èé%`ç\"'-some_thing", completion: { (user, cookie, _) -> Void in
            XCTAssertNotNil(cookie)
    ...
    func testIsLoggedIn() {
        ...
        HNLogin.shared.login(username: "abdurhtl", psw: "!Bullshit?Psw$", completion: { (_, _, _) -> Void in
